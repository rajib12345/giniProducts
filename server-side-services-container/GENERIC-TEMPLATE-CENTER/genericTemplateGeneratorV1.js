      ...
      if(params.length > 0){
        for (let i = 0; i < params.length; i++) {
            ...
            if(item[params[i]] !== null && item[params[i]] !== undefined && Array.isArray(item[params[i]]) === true){
              ...
              // if(item[params[i]] === null || item[params[i]] === undefined){
              //   console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
              //
              //   let homepage = 'https://gini-assistant.herokuapp.com';
              //   method += homepage;
              //   console.log("method : ", method);
              // }else{
              //   method += `'`+item[params[i]].toString()+`'`;
...
function getBlockTemplate(content, arguments, ellipseWidth, item){
  ...
  if(arguments.prefixValue !== null && typeof(arguments.prefixValue) === 'string' && arguments.prefixValue !== ''){
      ...
      content = '<b>'+arguments.prefixValue+' </b> '+content;
  }
  ...
  }else if(arguments.onclick === undefined){
  ...
  if(typeof(content) === 'string' && (content.length > 35)){
      ...
      if(arguments.ellipse === 'one line'){
        template = '<div class="ellipse " '+method+' style="width: '+ellipseWidth+'; white-space: nowrap; margin-top: 3px; ;'+customStyles+'" onclick="onOpenModal(\'' + content + '\', \'' +'type_text'+ '\')">'+content+'</div>';
        template += '<div class="tooltip">'+content+'</div>';
      }else if(arguments.ellipse === 'none'){
        if(arguments.api === "wikipedia_search"){
          template = '<div class="  " '+method+' style=" white-space: normal; margin-top: 3px;'+customStyles+'" >'+content+'</div>';

        }else{
          template = '<div class="  " '+method+' style=" white-space: normal; margin-top: 3px;'+customStyles+'" onclick="onOpenModal(\'' + content + '\', \'' + 'type_text' + '\')">'+content+'</div>';

        }
      }
  }else{
    // onclick="onOpenModal(\'' + content + '\', \'' + 'type_text' + '\')"
    template = '<div class="" '+method+' style=" white-space: normal; margin-top: 3px;'+customStyles+'" >'+content+'</div>';
  }
  return template;
}
  ...
  <div style="">
        <div class="">
          <div class="row" style="`+customStyle+`">
              ...
              <div class="col-lg-9 col-md-9 col-sm-9 col-xs-9 col-9" style="padding:1rem; `+descriptionTemplateStyle+`">
