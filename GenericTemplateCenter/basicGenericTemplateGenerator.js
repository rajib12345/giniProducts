...
function getViewAllSlideWithButtonTemplate(param){
  let template = '';
  let method = 'onclick="onClickSideMenu(\'' + param.btnAction + '\')"';

  template += `
  <div style="height: 100%;">
      <div id="" style="width:200px; height: auto; margin-top: 40%; margin-left: 50%;">
          <button type="button" class="btn" `+method+` style="width: 50px;height: 50px;border-radius: 50%;box-shadow: 1px 1px 4px 1px dimgrey;`+param.style+`;">
              <span class="glyphicon glyphicon-arrow-right" style="color: #607D8B; font-size: 20px; "></span>
          </button>
          <span style="display: block; color: #607D8B; margin-top: 5px;">`+param.btnName+`</span>
      </div>
  </div>`;
  ...
  if(Array.isArray(payload.data)){
    data.forEach(function(item, itemIndex){
      ...
      if(payload.emptySlide === 'yes'){
          ...
          let tmpExtraSlideTemplate = hScrollBar(extraSlideTemplate);
          template += tmpExtraSlideTemplate;
      }else if(payload.isViewAllBtnSlide !== null && payload.isViewAllBtnSlide !== undefined && payload.isViewAllBtnSlide !== false){
          let extraSlideTemplate = getViewAllSlideWithButtonTemplate(payload.isViewAllBtnSlide);
          let tmpExtraSlideTemplate = hScrollBar(extraSlideTemplate);
          template += tmpExtraSlideTemplate;
